...
(function ($) {
    ...
    // 02. mobile menu Js //
	let siMenuWrap = $('.ew-mobile-menu-active').find('ul').first().clone();
	let siSideMenu = $('.ew-offcanvas-menu nav');
	if (siMenuWrap.length) {
		...
		$(sideMenuList).on('click', function (e) {
			...
			$(this).siblings('.submenu, .mega-menu, .ew-submenu').slideToggle();
		});
	}
	...
	// 11. Open Handlers //
	$(".ew-offcanvas-open-btn").on("click", function () {
		$(".ew-offcanvas-area, .ew-offcanvas-2-area, .body-overlay").addClass("opened");
	});
	
	$(".ew-search-open-btn").on("click", function () {
		$(".ew-search-area, .body-overlay").addClass("opened");
	});
	...
	// 12. Close Handlers //
	$(".ew-offcanvas-close-btn, .ew-offcanvas-2-close-btn, .ew-search-close-btn, .cartmini-close-btn, .body-overlay").on("click", function () {
		$(".ew-offcanvas-area, .ew-offcanvas-2-area, .ew-search-area, .cartmini__area").removeClass("opened cartmini-opened");
		$(".body-overlay").removeClass("opened");
	});

	// 13. overlay not working js //
	$('.ew-offcanvas-open-btn').on('click', function () {
		const hasOffcanvas2 = $('.ew-offcanvas-2-area').length;
	...
	// 16. button hover animation //
	$('.ew-circle-btn').on('mouseenter', function (e) {
		var x = e.pageX - $(this).offset().left;
		var y = e.pageY - $(this).offset().top;

		$(this).find('.ew-circle-btn-dot').css({
			top: y,
			left: x
		});
	});
  	...
  	// 17. Button Move Animation //
	const all_btn = gsap.utils.toArray(".ew-circle-btn-wrap");
	...
	if ($('.ew-char-animation').length > 0) {
		let char_come = gsap.utils.toArray(".ew-char-animation");
	...
	// 22. ew-text-revel-anim //
	const si_anim_reveal = document.querySelectorAll(".ew-text-revel-anim");
	si_anim_reveal.forEach(areveal => {
		...
		areveal.split = new SplitText(areveal, { type: "lines,words,chars", linesClass: "ew-revel-line" });
    ...
    // 23. Service Hover Animation Js
    $('.ew-service-list-wrap .ew-service-list-item').on("mouseenter", function () {
        $('#ew-service-thumb').removeClass().addClass($(this).attr('rel'));
	...
	tm.add("(min-width: 991px)", () => {
		...
		let panels = document.querySelectorAll('.ew-project-2-area')
		panels.forEach((section, index) => {
			tl_team.to(section, {
				scrollTrigger: {
					trigger: '.ew-project-2-area',
					pin: '.ew-project-2-title-box',
					scrub: 1,
					start: 'top 17%',
					end: "bottom 90%",
					endTrigger: '.ew-project-2-area',
					pinSpacing: false,
					markers: false,
				},
			})
